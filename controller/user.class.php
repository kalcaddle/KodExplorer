...
class user extends Controller
{
    ...
    public function loginCheck()
    {
        ...
        if (in_array(ST, $this->notCheckApp) && in_array(ACT, $this->notCheck)) {
            return;
        }//不需要判断的控制器 | 不需要判断的action
        ...
        if (isset($_SESSION['kod_login']) && $_SESSION['kod_login'] === true) {
            ...
            if ($_COOKIE['kod_user_id'] != '' && $_COOKIE['kod_token'] != '') {
                ...
                if ($this->make_login_token($user) == $_COOKIE['kod_token']) {
                    ...
                    if (!isset($_SESSION['kod_user']) || !is_array($_SESSION['kod_user'])) {
                        $this->login(DATA_PATH."<br/>".$GLOBALS['L']['path_can_not_write_data']);
                    } else {
                        $this->login_success($user);
                    }
    ...
    private function login_success($user)
    {
        ...
        if (!isset($this->config['user']['file_repeat']) || !isset($this->config['user']['resize_config'])) {
    ...
    public function sso()
    {
        ...
        if (isset($_SESSION) && $_SESSION['kod_login'] == 1) {//避免session不可写导致循环跳转
            ...
            if ($user['role'] == '1' || !isset($this->in['check']) || !isset($this->in['value'])) {
                $result = true;
            }
            ...
            switch ($this->in['check']) {
                case 'user_id':
                    $check_value = $user['user_id'];
                    break;
                case 'user_name':
                    $check_value = $user['name'];
                    break;
                case 'role_id':
                    $check_value = $user['role'];
                    break;
                case 'role_name':
            ...
            if (!$result && $check_value != false) {
                if ((is_string($check_value) && $check_value == $this->in['value']) || (is_array(
                            $check_value
                        ) && in_array($this->in['value'], $check_value))
                ) {
                    $result = true;
                } else {
    ...
    public function loginFirst()
    {
        if (!file_exists(USER_SYSTEM.'install.lock')) {
            ...
            $user = [//重置admin
                ...
                'user_id' => $root,
                'role' => '1',
                'config' => ['size_max' => '0', 'size_use' => 1024],
                'group_info' => ['1' => 'write'],
                'create_time' => time(),
                'status' => 1,
            ];
            ...
            if (!$user['create_time'] || !$user['path'] || !file_exists(USER_PATH.$user['path'])) {
    ...
    public function loginSubmit()
    {
        ...
        if (isset($this->in['login_token'])) {
            ...
            if (strlen($api_token) < 5 || count($param) != 2 || md5(base64_decode($param[0]).$api_token) != $param[1]) {
            ...
            if (need_check_code() && $this->in['name'] != 'guest' && $_SESSION['check_code'] !== strtolower(
                    $this->in['check_code']
                )
            ) {
    ...
    {
        $not_check = ['user:common_js'];
        if (!$this->config['setting_system']['csrf_protect'] || isset($this->in['access_token']) || in_array(
                ST.':'.ACT, $not_check
            )
        ) {
            return;
        ...
        if (!isset($_SERVER['HTTP_X_CSRF_TOKEN']) || $_SERVER['HTTP_X_CSRF_TOKEN'] != $_SESSION['X-CSRF-TOKEN']) {
    ...
    {
        if (in_array(ST, $this->notCheckApp)) {
            return;
        }//不需要判断的控制器
