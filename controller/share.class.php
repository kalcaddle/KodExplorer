...
class share extends Controller
{
    ...
    public function __construct()
    {
        ...
        //需要检查下载权限的Action
        $arr_check_download = ['fileDownload', 'zipDownload'];//'fileProxy','fileGet'
    ...
    private function _check_share()
    {
        ...
        //检查是否过期
        if (isset($share_info['time_to']) && $share_info['time_to'].length != 0) {
        ...
        if (!isset($this->in['password'])) {
            //输入密码
            if ($_SESSION['password_'.$this->in['sid']] == $share_info['share_password']) {
                return;
            }
            $this->error('password');
        } else {
    ...
    public function officeView()
    {
        ...
        if (defined("OFFICE_KOD_SERVER")) {
            ...
            $office_url = OFFICE_KOD_SERVER.rawurlencode(
                    $file_url
                ).'&lang='.LANGUAGE_TYPE.'&appType=desktp&appMode=view'.'&file_time='.@filemtime(
                    $this->path
                ).'&key='.md5($this->path).'&app_id='.OFFICE_KOD_APP_ID.'&app_s='.$app_r.'&app_v='.md5(
    ...
    public function fileGet()
    {
        ...
        if (!json_encode(["data" => $filecontents])) {
