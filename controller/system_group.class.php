...
class system_group extends Controller
{
    ...
    private function _init()
    {
        ...
        $default = [
            '1' => [
                ...
                'children' => '',
                'config' => [
                    'size_max' => floatval(1.5),
                    'size_use' => floatval(1024 * 1024),
                ],//总大小，目前使用大小
                'path' => 'root',
                'create_time' => time(),
            ],
    ...
    public function add()
    {
        if (!isset($this->in['name']) || //必填项
            !isset($this->in['parent_id']) || !isset($this->in['size_max'])
        ) {
            show_json($this->L["data_not_full"], false);
        }
        ...
        $group_info = [
            ...
            'children' => '',
            'config' => [
                'size_max' => floatval($this->in['size_max']),//G
                'size_use' => floatval(1024 * 1024),
            ],//总大小，目前使用大小
    ...
    public function edit()
    {
        ...
        if (isset($this->in['parent_id']) && $group_info['parent_id'] != '' && //根目录不能修改父节点
            $this->in['parent_id'] != $group_info['parent_id']
        ) {//父节点变更
    ...
    public function del()
    {
        if (!isset($this->in['group_id'])) {
            show_json($this->L["data_not_full"], false);
        }

        if (strlen($this->in['group_id']) <= 1) {
        ...
        $this->sql->set(//将该节点的子节点的父节点设置为根目录
            ['parent_id', $group_info["group_id"]], ['parent_id', '1']
        );
