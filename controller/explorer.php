...
class explorer extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (isset($this->in['path'])) {
            ...
            $path_from_not_check = ['explorer:unzip'];
    ...
    private function check_system_path()
    {
        ...
        if (($GLOBALS['path_type'] == KOD_USER_SHARE) && !strstr(trim($this->in['path'], '/'), '/')) {//分享根目录
    ...
    public function pathInfo()
    {
        ...
        if (count($info_list) == 1 && $info_list[0]['type'] != 'folder') {//单个文件
            ...
            if ($GLOBALS['is_root'] || $GLOBALS['auth']['explorer:fileDownload'] == 1 || isset($this->in['viewPage'])) {
            ...
            if (in_array($ext, ['jpg', 'gif', 'png', 'jpeg', 'bmp'])) {
    ...
    public function pathChmod()
    {
        ...
        foreach ($info_list as $val) {
            ...
            chmod_path($path, $mod) ? $success++ : $error++;
        }
    ...
    public function search()
    {
        if (!isset($this->in['search'])) {
            show_json($this->L['please_inpute_search_words'], false);
        }
        ...
        //共享根目录不支持搜索
        if ($GLOBALS['path_type'] == KOD_USER_SHARE && strstr($this->path, KOD_USER_SHARE)) {
            show_json($this->L['path_cannot_search'], false);
        }

        $list = path_search(
            $this->path, iconv_system(rawurldecode($this->in['search'])), $is_content, $ext, $is_case
        );
    ...
    public function pathList()
    {
        ...
        //群组根目录
        if ($list['info']['path_type'] == KOD_GROUP_PATH && !strstr(
                trim(_DIR_CLEAR($this->in['path']), '/'), '/'
            )
        ) {//自己的根目录
    ...
    {//树结构
        ...
        if ((isset($this->in['tree_icon']) && $this->in['tree_icon'] != 'groupPublic') &&  //公共目录刷新排除
            !strstr(
                trim(rawurldecode($this->in['path']), '/'), '/'
            ) && ($GLOBALS['path_type'] == KOD_GROUP_PATH || $GLOBALS['path_type'] == KOD_GROUP_SHARE)
        ) {
...
//        }
    ...
    private function _tree_init($app)
    {
        if ($app == 'editor' && isset($this->in['project'])) {
            ...
            $tree_data = [
                [
                    'name' => get_path_this($this->in['project']),
                    'children' => $project,
        ...
        $root_isparent = count($root) ? true : false;
        $public_isparent = count($public) ? true : false;
        $tree_data = [
            'fav' => [
                'name' => $this->L['fav'],
                'ext' => "treeFav",
        ...
        foreach ($tree_data as $key => $value) { //为空则不展示
            if (count($value['children']) < 1 && in_array($key, ['my_group', 'group'])) {//'fav'
                continue;
    ...
    private function _group_tree($node_id)
    {//获取组织架构的用户和子组织；为空则获取根目录
        $group_sql = system_group::load_data();
        $groups = $group_sql->get(['parent_id', $node_id]);
        $group_list = $this->_make_node_list($groups);

        if ($node_id == '1' && !$this->config['setting_system']['root_list_group']) {//根群组不显示子群组
            $group_list = [];
        }

        if ($node_id == '1' && !$this->config['setting_system']['root_list_user']) {//根群组不显示用户
            return $group_list;
        }
    ...
    public function pathDelete()
    {
        ...
        if (!path_writeable($user_recycle) || isset($this->in['shiftDelete'])) {//回收站不可写则直接删除；传入直接删除参数
            $remove_to_recycle = '0';
        }
        ...
        foreach ($list as $val) {
            ...
            //不是自己目录的分享列表，不支持删除
            if ($GLOBALS['path_type'] == KOD_USER_SHARE && $GLOBALS['path_id'] != $_SESSION['kod_user']['user_id'] && substr_count(
                    trim($val['path'], '/'), '/'
                ) <= 1
            ) { //分享根项目
            ...
            if ($remove_to_recycle != "1" || $GLOBALS['path_type'] == KOD_USER_RECYCLE) {//回收站删除 or 共享删除等直接删除
                if ($val['type'] == 'folder') {
                    if (del_dir($path_this)) {
                        $success++;
                    } else {
                        $error++;
                    }
                } else {
                    if (del_file($path_this)) {
                        $success++;
                    } else {
                        $error++;
                    }
    ...
    public function pathCopyDrag()
    {
        ...
        foreach ($clipboard as $val) {
            ...
            if ($this->in['filename_auto'] == 1 && trim($auto_path, '/') == trim($path_copy, '/')) {
    ...
    public function pathPast()
    {
        if (!isset($_SESSION['path_copy'])) {
            show_json($this->L['clipboard_null'], false, []);
        }
        ...
        if (!path_writeable($path_past)) {
            show_json($this->L['no_permission_write'], false, $data);
        }

        if ('copy' != $copy_type) {
            $_SESSION['path_copy'] = json_encode([]);
    ...
    {
        if (filesize($this->path) <= 1024 * 50 || !function_exists('imagecolorallocate')) {//小于50k或者不支持gd库 不再生成缩略图
        ...
        if (!file_exists($image_thum) || filesize($image_thum) < 100) {//缩略图生成失败则使用原图
            $image_thum = $this->path;
        }
    ...
    public function serverDownload()
    {
        ...
        if ($this->in['type'] == 'percent') {//获取下载进度
            ...
            if ($this->in['type'] == 'remove') {//取消下载;文件被删掉则自动停止
        ...
        if ($result['code']) {
            get_path_this(iconv_app($save_path));
    ...
    public function officeView()
    {
        ...
        if (defined("OFFICE_KOD_SERVER")) {
            ...
            $office_url = OFFICE_KOD_SERVER.rawurlencode(
                    $file_link
                ).'&lang='.LANGUAGE_TYPE.'&appType=desktop'.$view_type.'&file_time='.@filemtime(
                    $this->path
                ).'&key='.md5(
                    $this->path
                ).'&user_id='.$user_info['user_id'].'&user_name='.$user_info['name'].'&app_id='.OFFICE_KOD_APP_ID.'&app_s='.$app_r.'&app_v='.md5(
                    $app_r.OFFICE_KOD_APP_KEY
                );
        ...
        if (strpos(OFFICE_SERVER, 'view.officeapps.live.com') === -1 || strstr($host, '10.10.') || strstr(
                $host, '192.168.'
            ) || strstr($host, '127.0.') || !strstr($host, '.')
        ) {
    ...
    public function officeSave()
    {
        ...
        if (!path_writeable($save_path)) {
            $this->json_putout(['error' => 'no_permission_write']);
        }

        if (($body_stream = file_get_contents('php://input')) === false) {
            $this->json_putout(['error' => 'Bad Request']);
        }

        $data = json_decode($body_stream, true);

        if (is_null($data)) {
            $this->json_putout(['error' => 'Bad Response']);
        }
        ...
        $result = ['error' => 0, 'action' => $_trackerStatus[$data["status"]]];
    ...
    private function json_putout($info)
    {
        ...
        if (GLOBAL_DEBUG) {
            write_log(json_encode([$this->in, $info]), 'office_save');
        }
    ...
    private function path_fav(&$list)
    {
        ...
        foreach ($fav_list as $key => $val) {
            $the_path = _DIR($val['path']);
            $has_children = path_haschildren($the_path);
            ...
            if (strstr($val['path'], KOD_USER_SHARE) || strstr($val['path'], KOD_USER_FAV) || strstr(
                    $val['path'], KOD_GROUP_ROOT_SELF
    ...
    {
        $data_list = KOD_GROUP_ROOT_SELF == $group_root_type
            ? $this->_group_self()
            : $this->_group_tree(1);
    ...
    private function path($dir, $list_file = true, $check_children = false)
    {
        ...
        if ($dir === false) {
            ...
            if ($GLOBALS['path_type'] == KOD_USER_SHARE && !strstr(
                    trim($this->in['path'], '/'), '/'
                )
            ) {//分享根目录 {user_share}:1/ {user_share}:1/test/
                $list = $this->path_share($list);
            } else {
                if ($GLOBALS['path_type'] == KOD_USER_FAV) {//收藏根目录 {user_fav}
                    $list = $this->path_fav($list);
                } else {
                    if ($GLOBALS['path_type'] == KOD_GROUP_ROOT_SELF) {//自己用户组目录；KOD_GROUP_ROOT_SELF
                        $list = $this->path_group($list, $GLOBALS['path_type']);
                    } else {
                        if ($GLOBALS['path_type'] == KOD_GROUP_ROOT_ALL) {//全部用户组目录；KOD_GROUP_ROOT_ALL
                            $list = $this->path_group($list, $GLOBALS['path_type']);
                        } else {
    ...
    private function _role_check_info(&$list)
    {
        if (!$GLOBALS['path_type']) {
            $list['info'] = ["path_type" => '', "role" => '', "id" => '', 'name' => ''];
        ...
        //自己管理的目录
        if ($GLOBALS['path_type'] == KOD_GROUP_PATH || $GLOBALS['path_type'] == KOD_GROUP_SHARE) {
