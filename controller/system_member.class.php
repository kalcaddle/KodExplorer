...
class system_member extends Controller
{
    ...
    public function add()
    {
        if (!isset($this->in['name']) || //必填项
            !isset($this->in['password']) || !isset($this->in['role']) || !isset($this->in['group_info']) || //{"0":"read","100":"read"}
        ...
        foreach ($user_array as $val) {
            ...
            $user_info = [
                ...
                'role' => $this->in['role'],
                'config' => [
                    'size_max' => floatval($this->in['size_max']),//M
                    'size_use' => 1024 * 1024,
                ],//总大小，目前使用大小
    ...
    public function edit()
    {
        ...
        //管理员自己不能添加自己到非管理员组
        if ($GLOBALS['is_root'] && $_SESSION['kod_user']['user_id'] == $user_id && $this->in['role'] != '1') {
            show_json($this->L['error'], false);
        }
        ...
        if ($user_info['name'] != $the_name) {
            if ($this->sql->get(['name', $the_name])) {
                show_json($this->L['error_repeat'], false);
            }
        }

        $this->in['name'] = rawurlencode($the_name);//还原
        $edit_arr = ['name', 'role', 'password', 'group_info', 'home_path', 'status', 'size_max'];
    ...
    {
        $user_folder = ['home', 'recycle_kod', 'data'];
