define(function(require, exports) {
	...
	var _bindSelectEvent = function(){
		...
		var startY			= null;
		var $selectDiv		= null;
		var offset_top  	= 85+Global.topbar_height;
		...
		var __dragSelectInit = function(e) {
			mainScrollTop = $(Config.BodyContent).scrollTop();
			...
			if ($(e.target).hasClass("ico")==false){// 编辑状态不可选
				...
				//$selectDiv = $('<div id="selContainer"></div>').appendTo(Config.FileBoxSelector);
				startX = e.pageX - Global.frameLeftWidth;
		...
		var __dragSelecting= function(e) {
			...
			var scroll_cute = $(Config.BodyContent).scrollTop() - mainScrollTop;
			...
			// ---------------- 框中选择关键算法 ---------------------
			var _l = $selectDiv.offset().left - Global.frameLeftWidth;
	...
	var fileLight = {	
		...
		resumeMenu:function($obj){
			var menu = {"fileApp":"menuApp","fileBox":"menufile","folderBox":"menufolder"};
			for(var key in menu){
				if ($obj.hasClass(key)) {
		...
		getAllName:function(){
			...
			$list.each(function(i){
				arr_name[i] = fileLight.name($(this));
			});
			return arr_name;
		},
